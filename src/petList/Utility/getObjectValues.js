...
const getUniqueAttributeValue = (ownerDetails, attributeName) => {
  ...
  if (ownerDetails && attributeName) {
    uniqueAttributeValue = [
      ...new Set(ownerDetails.map((item) => item[attributeName]))
    ]
  }
...
const getPetList = (
  ...
  if (ownerDetails && ownerAttribute) {
    let petNames = []
    const petList = ownerDetails.filter((item) => {
      return item[ownerAttribute] === ownerAttributeValue
    })?.map(item =>{ if(item?.pets?.length) return item?.pets})
    if (petList?.length) {
      petList.forEach(element => {
        petNames.push(element?.map((item)=>{if(item?.type?.toLowerCase() === petCategory) return  item.name}))
      });
    }
